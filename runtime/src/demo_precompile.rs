	...
	Runtime: pallet_revive::Config,
{
	...
	type Interface = IDemo::IDemoCalls;
	...
	fn call(
		...
		match input {
			IDemoCalls::echo(IDemo::echoCall { mode, message }) => {
				if *mode == 0 {
					log::error!(target: LOG_TARGET, "reverting");
					return Err(Error::Revert("mode was set to 0".into()))
				}

				log::debug!(target: LOG_TARGET, "echoing message back {message:?}");
